    ...
    UIMainScene.IUIInfoContent
{
    ...
    private void Start()
    {
        selectedColorString = PlayerPrefs.GetString("Color");

        string redstring = selectedColorString.Substring(5, 5);
        float redvalue = float.Parse(redstring);

        string greenstring = selectedColorString.Substring(12, 5);
        float greenvalue = float.Parse(greenstring);

        string bluestring = selectedColorString.Substring(19, 5);
        float bluevalue = float.Parse(bluestring);

        string alphastring = selectedColorString.Substring(26, 5);
        float alphavalue = float.Parse(alphastring);

        selectedColor = new Color(redvalue, greenvalue, bluevalue, alphavalue);
